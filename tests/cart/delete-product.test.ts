import {CART_PATH} from "../../src/cart/cart.controller";
import {Mongo} from "../../src/db/mongo";
import {appConfig} from "../../src/utils/config";
import supertest from "supertest"
import {appMock} from "../mocks/app.mock";
import {cartController} from "../tests.utils/controllers";
import {productBody1, productInCart2} from "../tests.utils/insert.product";
import {cart1, insertOne} from "../tests.utils/insert.cart";
import {CartRepository} from "../../src/repository/cart.repository";

describe(`POST ${CART_PATH}/delete-product`, () => {
    beforeAll(async () => {
        await Mongo.connect(appConfig.MONGO_URL)
        await insertOne(cart1)
    })

    afterAll(async () => {
        await Mongo.cart().drop()
        await Mongo.close()
    })

    it('should delete product from cart', async () => {
        const deleteProductResponse = await supertest(appMock(cartController))
            .post(`${CART_PATH}/delete-product`)
            .send(productBody1)

        const cart = await CartRepository.findOne(cart1.cartId)

        expect(deleteProductResponse.status).toBe(200)

        expect(cart!.products).toStrictEqual([productInCart2]);
        expect(cart!.totalValue).toBe(cart1.totalValue - productBody1.price * productBody1.quantity)
    })
})